...
combs = []
for tag_per_image in all_tags_list:
    for comb in combinations(tag_per_image, 2):
        combs.append(comb)
...
def image_page():
    ...
    if "," in tag:
        ...
        images_with_tag = [img for img in all_images if tag1 in img["tags"] and tag2 in img["tags"]]
    else:
...
def stats_page(min_tags_number=3):
    filtered_tags = {
        k: v
    ...
    st.markdown(f"#### Top 3 Tags")

    df = pd.DataFrame(list(zip(labels[:3], counts[:3])), columns=["Tags", "Counts"])
